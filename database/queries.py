...
def db_create(data):
    ...
    cur = conn.execute(
        ...
        (name, age, gender, phone, email, disease, doctor, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (data["name"], data["age"], data["gender"], data["phone"],  data["email"], data["disease"], data["doctor"],  now)
...
def db_update(patient_id, data):
    ...
    conn.execute(
        """
        UPDATE patients
        SET name=?, age=?, gender=?, phone=?, email=?, disease=?, doctor=?, updated_at=?
        WHERE id=?
        """,
        (data["name"], data["age"], data["gender"], data["phone"],  data["email"], data["disease"], data["doctor"],  now,  patient_id))
    conn.commit()
    conn.close()
